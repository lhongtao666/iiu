...
}}}}return b;}};var CashSettingUtils={getOptions:function(b){var a=this;var c=window.sessionStorage.getItem("loacalCashSetting");if(c&&c!=null&&c!=""){a._apendData(b,JSON.parse(c));
}else{FormUtils.loadData(ctx+"/gl/cash/cashSetting/listData.htm",function(d){window.sessionStorage.setItem("loacalCashSetting",JSON.stringify(d));a._apendData(b,d);
});}},_apendData:function(a,c){var d;if(a.container){d=$("#"+a.container);if(d.length==0){d=$("select."+a.container);if(d.length==0){d=$("select[name='"+a.container+"']");
if(d.length==0){return false;}}}}var e=new StringBuffer();if(a.caption){e.append("<option value=''>").append(a.caption).append("</option>");}if(a.emptyOptions){e.append("<option value='"+a.emptyOptions+"'>").append("").append("</option>");
}if(c&&c.length>0){for(var b=0;b<c.length;b++){e.append("<option value='"+c[b].id+"'>"+c[b].name+"</option>");}d.empty().append(e.toString());}}};function SendMessage(){this.hadInit=false;
}SendMessage._CONS_={LIST_DATA_URL:ctx+"/gl/sms/smsRecord/findByCsId.htm",SEND_MSG_URL:ctx+"/gl/sms/smsRecord/sendMsg.htm",SEND_ALI_TEMPLATE_URL:ctx+"/gl/sms/smsRecord/sendAliTemplateMsg.htm",RE_SEND_MSG_URL:ctx+"/gl/sms/smsRecord/resend.htm",JUDGE_IS_PHONE_URL:ctx+"/crm/cs/csEntity/judgeIsPhone.htm",BATCH_SEND_MSG_URL:ctx+"/gl/sms/smsRecord/batchSendMsg.htm",DIV:"#smsRecordGridDiv",GRID:"#smsRecordGrid",PAGER:"#smsRecordPager",GRID_ID:"smsRecordGrid",PAGER_ID:"smsRecordPager"};
...
s<t.length;s++){$("input[name='listTitleCheck'][value='"+t[s]+"']").prop("checked",true);}}else{$("input[name='listTitleCheck']").prop("checked",true);
}});}},on:function(k,j){layer.close(a.layerIndex);},cancel:function(){layer.close(a.layerIndex);}});}}});}};$(function(){var a=new JtGridTip();a.init();
});function JtGridTip(){}JtGridTip.prototype={init:function(){this._initContainerDiv();this._bindEventHander();},_initContainerDiv:function(){if($("#jt-grid-tip-div").length<=0){$("body").append("<div id='jt-grid-tip-div'></div>");
}},_bindEventHander:function(){var a=this;$(document).on("mouseenter",".jt-grid-tip",function(d){var c=$(this).attr("data-value");$("#jt-grid-tip-div").empty().text(c);
var b=$(this).offset().left;var f=$(this).offset().top+5-$("body").height()*0.025;if(f+300>$(document).height()){f=$(document).height()-300;}$("#jt-grid-tip-div").css({"top":f,"left":b});
...
},function(b){$("#jt-grid-tip-div").stop().hide("fast");});}};var SoEntityUtils={formatStatus:function(e){var b="";var a=e.status;var d=e.delayConfirmTime;
var c=e.delayShipTime;if(a=="awaiting_pay"){b="待支付";}else{if(a=="awaiting_post"){b="待提交";}else{if(a=="awaiting_comfirm"){if(d!=null){b="待审核"+"<span style='color:red;font-size: 8px;'>[延审]</span>";
}else{b="待审核";}}else{if(a=="comfirm"){if(c!=null){b="待发货"+"<span style='color:red;font-size: 8px;'>[延发]</span>";}else{b="待发货";}}else{if(a=="shipping"){b="已发货";
}else{if(a=="complete"){b="已签收";}else{if(a=="close"){b="已作废";}else{if(a=="close_reject"){b="已拒收";}else{if(a=="close_lost"){b="已丢失";}}}}}}}}}return b;},formatCashType:function(b){var a="";
if(b=="pay"){a="款到发货";}else{if(b=="pay_sub"){a="支付订金";}else{if(b=="green"){a="绿色通道";}else{if(b=="green_by"){a="绿色通道订金";}else{if(b=="cod"){a="货到付款";}else{if(b=="free"){a="免费已付";
}else{if(b=="point"){a="积分付款";}}}}}}}return a;},formatPayType:function(b){var a="";if(b=="alipay"){a="支付宝";}else{if(b=="weixin"){a="微信支付";}else{if(b=="weixinhongbao"){a="微信红包";
...
}}}});});});});});},};function parentCMD(){$(".soEntitySearch").trigger("click");}function SoEntityManager(){this.simpleShowLastFollow=(simpleShowLastFollow=="n"?false:true);
this.simpleShowComment=(simpleShowComment=="n"?false:true);this.simpleShowProduct=(simpleShowProduct=="n"?false:true);this.isUseReturnBack=(isUseReturnBack=="n"?false:true);
}SoEntityManager.prototype={_initSearch:function(){new FormConfig({"formId":$("#tabSoAll").find(".soEntitySearchForm"),entry:$("#tabSoAll").find(".jt-form-config"),title:"我的订单查询配置",isHidden:true,forceParam:["客户号码"],"endLess":[],modular:"mySoEntitySearchList"}).init();
...
}break;}});},};$(function(){var a=new SoEntityManager();a.init();});var glHelpUtil={init:function(a){this._createGlHelpTag();this._bindEventHandler(a);
},_createGlHelpTag:function(){var a=new StringBuffer();a.append("<a style='margin-left:10px' title='帮助'><img src='/t9-ecm/img/jt/help.png'></a>");$("#glHelpContainer").empty().append(a.toString());
},_bindEventHandler:function(a){$(document).on("click","#glHelpContainer",function(){FormUtils.loadData(ctx+"/gl/km/glHelp/findGlHelpData.htm"+"?cateKey="+a,function(b){JTTabUtils.addOrRefreshTab(b.url,"<"+b.name+"> "+"帮助文档");
...
},cancel:function(){layer.close(a.layerIndex);}});$(this).attr("disabled",false);});}};function FormConfig(a){if(!a){a={};}this.title=a.title?a.title:"表单字段配置";
if(a.formId){if(typeof a.formId=="string"){this.formId=$("#"+a.formId);}else{if(typeof a.formId=="object"){this.formId=a.formId;}}}else{console.log("无法识别的表单对象！");
this.isError=true;return false;}if(a.entry){if(typeof a.entry=="string"){this.entry=$("#"+a.entry);}else{if(typeof a.entry=="object"){this.entry=a.entry;
