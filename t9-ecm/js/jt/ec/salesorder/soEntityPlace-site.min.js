...
c.chosen();}else{c.select2();}},null,true);},};$(function(){var a=new SoEntityPlace();a.init();});function SoEntityPlace(){this.soEntityBase=new SoEntityBase();
}SoEntityPlace._CONS_={SAVE_ADD_URL:ctx+"/ec/salesorder/soEntity/saveAdd.htm",SAVE_EDIT_URL:ctx+"/ec/salesorder/soEntity/saveEdit.htm",FIND_THREE_DAYS_ORDER_URL:ctx+"/ec/salesorder/soEntity/findByCsIdAndTime.htm",FIND_GROUP_NOORDERTIME_URL:ctx+"/gl/ou/group/groupNoOrdertime.htm",};
...
$("#orderListDiv").stop().slideDown();}});},};var FinanceAccountSelect={initSelect:function(c,b,a){$.ajax({type:"GET",url:ctx+"/ec/finance/financeAcount/listPo.htm",contentType:"application/x-www-form-urlencoded",dataType:"json",success:function(f){if(f!=null&&f!=""){var d=document.getElementById(c);
