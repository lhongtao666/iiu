...
}BatchSendWxMsg._CONS_={COUNT_FRIEND_URL:ctx+"/gl/cs/weiXinFriends/countWxFriends.htm",BATCH_SEND_URL:ctx+"/crm/cs/weiXinRecord/batchSend.htm",FIND_FACE_IMAGES_URL:ctx+"/crm/cs/weiXinChat/findMyImage.htm",COLLECTION_IMG_URL:ctx+"/crm/cs/weiXinChat/collectionImage.htm",FIND_COM_LAN_URL:ctx+"/crm/cs/weixinCommonLan/findByUserId.htm",UPLOAD_PARAMS:{"acceptTypes":".*","mimeTypes":".*","fileSingleSizeLimit":10*1024*1024*1,"fileNumLimit":10,"uploadUrl":ctx+"/crm/components/upload/ajax.htm"}};
...
$(".web_wechat_common_lan").removeClass("on");},_hideNotGrantBtn:function(){},_bindEventHander:function(){var a=this;$(document).on("click",".batch-send-label",function(){if($(this).hasClass("batch-send-label-selected")){$(this).removeClass("batch-send-label-selected");
...
});$(document).on("click",".jt-weixin-batch-sendMsgBtn",function(){a._send();});$(document).on("keydown","#batch-chat-textarea",function(b){if(b.keyCode==13){if(!b.ctrlKey){a._send();
}}});$(".web_wechat_face").on("click",function(){if($(this).hasClass("disabled")){return false;}if($(this).hasClass("on")){a._hideFaceDiv();}else{a._hideImageDiv();
a._hideComLanDiv();a._showFaceDiv();}});$(".web_wechat_image").on("click",function(){if($(this).hasClass("disabled")){return false;}if($(this).hasClass("on")){a._hideImageDiv();
}else{a._hideFaceDiv();a._hideComLanDiv();a._showImageDiv();}});$(".web_wechat_common_lan").on("click",function(){if($(this).hasClass("disabled")){return false;
...
}$("#filePicker").find(".webuploader-element-invisible").click();});$(".selectFirends").on("click",function(){var b=new WeiXinFriendsSelect();var c={isMultiple:1,callBack:function(g){if(g!=null){if(g.length>0){var j=new StringBuffer();
var f=0;for(var d=0;d<g.length;d++){if($("#"+g[d].id).length<=0){j.append("<li id='"+g[d].id+"' data-remark='"+g[d].remark+"' data-username='"+g[d].userName+"'> ").append("<a href='javascript: void(0);'>").append(g[d].remark).append("&nbsp;&nbsp;").append("<button class='btn btn-danger btn-xs deleteFriendsBtn' type='button'><i class='fa fa-remove'></i></button>").append("</a></li>");
...
this._sendFinal(c);},_sendFinal:function(b){var a=this;b.userNames=a._getUserNamse();if(b.userNames.length<=0){DialogUtils.error(msgCode.ERROR,"请选择要群发的好友");
return;}DialogUtils.confirmWithOptions({title:"确定给"+$("#selected-firends").val()+"位好友发送消息吗？",text:"",confirmButtonText:"确定",yesFunc:function(){FormUtils.submit(BatchSendWxMsg._CONS_.BATCH_SEND_URL,b,function(c){if(c.success){DialogUtils.success(msgCode.SUCCESS,"消息发送中...");
$("#batch-chat-textarea").empty();}else{DialogUtils.error(msgCode.ERROR,c.msg);}},function(){DialogUtils.error(msgCode.ERROR,msgCode.ERROR_MSG);},true);
}});},_getUserNamse:function(){var b=$(".tag-list").find("li");var c=[];for(var a=0;a<b.length;a++){c.push($(b[a]).attr("data-username"));}return c;},_findWxFriendNum:function(){var a=BatchSendWxMsg._CONS_.COUNT_FRIEND_URL;
...
$("#selected-firends").val(d.msg);}},function(){DialogUtils.error(msgCode.ERROR,msgCode.ERROR_MSG);},true);},_getLabels:function(){var b=$(".batch-send-label-selected");
var c="";for(var a=0;a<b.length;a++){if(a==0){c=$(b[a]).text();}else{c+=","+$(b[a]).text();}}return c;},_initWxlabel:function(){var a=this;var b=ctx+"/gl/cate/cate/getOptions.htm?typeKey=system&pKey=weixinLabel";
FormUtils.loadData(b,function(c){if(c.length>0){var e=new StringBuffer();for(var d=0;d<c.length;d++){e.append("<label class='btn btn-sm btn-info btn-outline batch-send-label' >"+c[d].value+"</label>");
}$(".batch-send-label-div").empty().append(e.toString());}});},_initFaceImages:function(){var a=BatchSendWxMsg._CONS_.FIND_FACE_IMAGES_URL;FormUtils.loadData(a,function(f){var d=f.myCollectImages;
var c=f.systemImages;var g=new StringBuffer();for(var b=0;b<d.length;b++){g.append("<img class='my-face-image face-image' id='"+d[b].id+"' src-value='"+d[b].path+"' src='"+ctx+d[b].path+"'></img>");
