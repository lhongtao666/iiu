function OnlineTextarea(){this.hadInit=false;this.container="";this.focus=false;this.browser={};}OnlineTextarea._CONS_={};OnlineTextarea.prototype={init:function(a){if(!this.hadInit){this.hadInit=true;
...
ierange=document.selection.createRange();ierange.pasteHTML(b);$(g).focus();}}},focusin:function(){var b=this;var c=$(b.container);if(window.getSelection){c.focus();
var a=window.getSelection();a.selectAllChildren(c[0]);a.collapseToEnd();}else{if(document.selection){var a=document.selection.createRange();a.moveToElementText(c[0]);
a.collapse(false);a.select();}}},getValue:function(){var g=[];var c=$(this.container).clone().find(".qqface").each(function(h,j){$(this).replaceWith($(this).attr("data-value"));
}).end().html();console.log(c);console.log(typeof c);if(c.length>0){if(c.indexOf("<img")!=-1){while(c.length>0){if(c.indexOf("<img")!=-1){if(c.indexOf("<img")!=0){var b=c.substring(0,c.indexOf("<img"));
var e={};e.contentType="text";e.content=this.replaceToRN(b);g.push(e);c=c.substring(c.indexOf("<img"),c.length);}else{var d=c.substring(c.indexOf("<img"),c.indexOf(">")+1);
c=c.substring(c.indexOf(">")+1,c.length);var e={};var a=$(d);var f=a.attr("src");if(a.attr("data-type")){e.contentType=a.attr("data-type");e.content=a.attr("file-path");
e.contents={fileName:a.attr("file-name"),fileSize:a.attr("file-size"),filePath:a.attr("file-path")};}else{if(a.is(".online-large-img")){e.contentType="image";
e.content=a.attr("data-src");}}g.push(e);}}else{var e={};e.contentType="text";e.content=this.replaceToRN(c);g.push(e);c="";}}}else{var e={};e.contentType="text";
e.content=this.replaceToRN(c);g.push(e);}}return g;},replaceToRN:function(a){a=a.replace(new RegExp("<br>","gm"),"\r\n").replace(new RegExp("&nbsp;","gm")," ");
return this.removeHtml(a);},replaceToBR:function(a){a=a.replace(new RegExp("\r\n","gm"),"<br>").replace(new RegExp(" ","gm"),"&nbsp;");return a;},removeHtml:function(a){a=a.replace(/<\/?[^>]*>/g,"");
return a;},_replaceBr:function(a){a=a.replaceAll("<br>","\r\n").replaceAll('<br type="_moz">',"").replaceAll("&nbsp;"," ");return this._removeHtml(a);},_removeHtml:function(a){a=a.replace(/<\/?[^>]*>/g,"");
return a;},_getBrowser:function(){if(navigator.userAgent.indexOf("MSIE")>0){this.browser.name="MSIE";this.browser.ie=true;}else{if(navigator.userAgent.indexOf("Firefox")>0){this.browser.name="Firefox";
this.browser.firefox=true;}else{if(navigator.userAgent.indexOf("Chrome")>0){this.browser.name="Chrome";this.browser.chrome=true;}else{if(navigator.userAgent.indexOf("Safari")>0){this.browser.name="Safari";
this.browser.safari=true;}else{if(navigator.userAgent.indexOf("Opera")>=0){this.browser.name="Opera";this.browser.opera=true;}else{this.browser.name="unknow";
}}}}}}};