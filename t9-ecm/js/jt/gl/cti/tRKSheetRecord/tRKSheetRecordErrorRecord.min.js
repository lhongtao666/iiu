$(function(){var a=new InsideLineSheetRecord();a.init();});function InsideLineSheetRecord(){this.hadInit=false;}InsideLineSheetRecord._CONS_={LIST_DATA_URL:ctx+"/gl/cti/tRKSheetRecord/listDataForRecordByReqType.htm?reqType="+reqType,EXPORT_DATA_URL:ctx+"/gl/cti/tRKSheetRecord/exportRecord.htm",HALF_ASYN_EXPORT_DATA_URL:ctx+"/gl/cti/tRKSheetRecord/halfAsynExportRecord.htm",GRID:"#insideLineSheetRecordGrid",PAGER:"#insideLineSheetRecordPager"};
InsideLineSheetRecord.prototype={init:function(a){if(!this.hadInit){this.hadInit=true;this._bindEventHander();this._initToolbarSelect();this._initEmployeeSelect("");
this._initBelongEmployeeSelect("");this._initToolbar();new Switchery(document.querySelector(".js-switch"),{size:"small"});this._initJqgrid(a);this._hideNotGrantBtn();
...
}).on("onUnsetSelectValue",function(g){b.currentProd={};});},_initToolbar:function(){var a=new StringBuffer();a.append('<form id="insideLineSheetRecordSearchForm" role="form" class="form-horizontal">');
a.append('<div class="form-group basic-group ">');a.append('<div class=" col-sm-4  basic-group-div">');a.append('<label class="control-label  col-sm-3">记录时间</label>');
a.append('<div class="col-sm-9">');a.append('<div class="input-daterange input-group">');a.append('<input class="laydate-icon form-control layer-date" name="startTime" placeholder="开始时间" onclick="laydate({istime: true, format: \'YYYY-MM-DD hh:mm:ss\'})">');
a.append('<span class="input-group-addon">到</span>');a.append('<input class="laydate-icon form-control layer-date" name="endTime" placeholder="结束时间" onclick="laydate({istime: true, start: laydate.now(0, \'YYYY/MM/DD 23:59:59\'), format: \'YYYY-MM-DD hh:mm:ss\'})">');
a.append("</div>");a.append("</div>");a.append("</div>");a.append('<div class="col-sm-4">');a.append('<label class="control-label col-sm-3">客户号码</label>');
a.append('<div class="col-sm-6">');a.append("<input class='form-control' name='csPhone'>");a.append("</div>");a.append("</div>");a.append("</div>");a.append('<div class="form-group basic-group ">');
a.append('<div class=" col-sm-4  basic-group-div">');a.append('<label class="control-label  col-sm-3">注册时间</label>');a.append('<div class="col-sm-9">');
a.append('<div class="input-daterange input-group">');a.append('<input class="laydate-icon form-control layer-date" name="csCreateStartTime" placeholder="yyyy-mm-dd hh:mm:ss" onclick="laydate({istime: true, format: \'YYYY-MM-DD hh:mm:ss\'})">');
a.append('<span class="input-group-addon">到</span>');a.append('<input class="laydate-icon form-control layer-date" name="csCreateEndTime" placeholder="yyyy-mm-dd hh:mm:ss" onclick="laydate({istime: true, start: laydate.now(0, \'YYYY/MM/DD 23:59:59\'), format: \'YYYY-MM-DD hh:mm:ss\'})">');
a.append("</div>");a.append("</div>");a.append("</div>");a.append('<div class="col-sm-4">');a.append('<label class="control-label col-sm-3">类型</label>');
a.append('<div class="col-sm-6">');a.append("<select class='form-control' name='RecFile'>");a.append("<option value=''>请选择</option>");a.append("<option value='1'>已接</option>");
a.append("<option value='0'>未接</option>");a.append("</select>");a.append("</div>");a.append("</div>");a.append('<div class="col-sm-4">');a.append('<label class="col-sm-2 control-label">去重</label>');
a.append('<div class="col-sm-10">');a.append('<div class="col-sm-3">');a.append('<input type="checkbox" value="y" name="wipeRepeat" class="js-switch">');
a.append("</div>");a.append('<div class="col-sm-9">');a.append('<button type="button" class="btn jt-btn-search insideLineSheetRecordSearch"  style="margin-right:10px;margin-left:4px;"><i class="fa fa-search"></i>&nbsp;搜索</button>');
a.append('<button type="button" class="btn jt-btn-empty assignBtn"><i class="fa fa-trash-o"></i>&nbsp;清洗</button>');a.append('<label id="glHelpContainer"></label>');
a.append("</div>");a.append("</div>");a.append("</div>");a.append("</div>");a.append("</form>");$("#insideLineSheetRecordSearchForm").replaceWith(a.toString());
},_locateToGlHelp:function(){var a="glHelp.dataAudit.tRKSheetRecordError";glHelpUtil.init(a);},_hideNotGrantBtn:function(){if(!ResUtils.canShow("trkSheetRecord_errorRecord.button.assign")){$("button.assignBtn").hide();
...
if(f&&f.length){var e=[];for(var h=0;h<f.length;h++){var l=f[h];var k=$(InsideLineSheetRecord._CONS_.GRID).jqGrid("getRowData",l);if(k.csId.trim().indexOf("未注册")!=-1){if(k.encryptNumber&&e.indexOf(k.encryptNumber)<0){e.push(k.encryptNumber);
}}}if(e.length){var j={};j.successFn=function(i){if(i.success){DialogUtils.success("成功!",i.msg,undefined,4);$("#assignContainer").modal("hide");a.reloadJqgrid();
}};var d=new CsAssign(e,"csPhone",g,undefined,j);d.init();}else{DialogUtils.error("错误","没有可供分配的号码");}}else{var g="start="+$("input[name='startTime']").val();
if($("input[name='endTime']").val()){g+="&end="+$("input[name='endTime']").val();}else{g+="&end="+new Date().current();}var j={};j.successFn=function(i){if(i.success){DialogUtils.success("成功!",i.msg,undefined,4);
$("#assignContainer").modal("hide");a.reloadJqgrid();}};var d=new CsAssign([],"clear",g,undefined,j);d.init();}});$("body").on("click",".csEntityInBtn",function(){JTTabUtils.addOrRefreshTab(ctx+"/crm/cs/csEntity/in.htm?phoneNum="+$(this).attr("encryptNumberVal")+"&encryptInNumber="+$(this).attr("encryptInNumberVal")+"&inNum="+$(this).attr("inNumVal"),"新客户");
...
}else{if(!ResUtils.canShow("trkSheetRecord_limitCallin.button.assign")){return"<span title='创建客户' class='' href='javascript: void(0);' encryptInNumberVal='"+k+"'  encryptNumberVal='"+l.encryptNumber+"'>"+m+"</span>";
...
}else{if(k.indexOf("PJ:2")!=-1){return"一般";}else{if(k.indexOf("PJ:3")!=-1){return"不满意";}}}return"无";}},{name:"recfile",align:"center",width:180,hidden:true,formatter:function(l,k,i){if(l){var j=c._generateCode();
...
}else{if(reqType&&reqType=="adnStaticReq"){a=InsideLineSheetRecord._CONS_.LIST_DATA_URL+"&"+adnStaticReqParamStr;}else{a=InsideLineSheetRecord._CONS_.LIST_DATA_URL;
