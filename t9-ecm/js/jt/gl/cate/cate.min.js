...
$("#cancelCateBtn").show();});$("#saveCateBtn").on("click",function(){a._save();});$("#cancelCateBtn").on("click",function(){$(this).hide();$(this).prev().hide();
$(this).prev().prev().show();FormUtils.disableForm("cateForm1");ButtonUtils.disableBtn("selectDefaultImageBtn");a._loadCateData(a.currentCate.id);});},_loadCateData:function(b){var a=this;
$("#editCateBtn").show();FormUtils.loadData(Cate._CONS_.LOAD_CATE_DATA+"?id="+b,function(c){a._showTab(c);$("input[name='id']").val(c.id);$("input[name='typeKey']").val(c.typeKey);
$("input[name='depth']").val(c.depth);$("input[name='hasChild']").val(c.hasChild);$("input[name='parentId']").val(c.parentId);$("input[name='userId']").val(c.userId);
$("input[name='createTime']").val(c.createTime);$("input[name='path']").val(c.path);$("input[name='name']").val(c.name);$("input[name='key']").val(c.key);
$("input[name='subName']").val(c.subName);$("input[name='iconUrl']").val(c.iconUrl);if(c.iconUrl){$("#defaultImgDiv").attr("src",ctx+"/js/zTree_v3/css/zTreeStyle/img/jt-diy/"+c.iconUrl+".png");
