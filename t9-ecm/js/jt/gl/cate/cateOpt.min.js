$(function(){var a=new CateOpt();a.init();});function CateOpt(){this.hadInit=false;this.isEdit=false;this.isFormDataChange=false;}CateOpt._CONS_={EDIT_URL:ctx+"/gl/cate/cateOpt/edit.htm",DETAIL_URL:ctx+"/gl/cate/cateOpt/detail.htm",DELETE_URL:ctx+"/gl/cate/cateOpt/delete.htm",SAVE_ADD_URL:ctx+"/gl/cate/cateOpt/saveAdd.htm",SAVE_EDIT_URL:ctx+"/gl/cate/cateOpt/saveEdit.htm",LIST_DATA_URL:ctx+"/gl/cate/cateOpt/listData.htm",LIST_CATE_URL:ctx+"/gl/cate/cateOpt/listOptCatesByCs.htm",EDIT_FORM_ID:"cateOptForm"};
CateOpt.prototype={init:function(b){if(!this.hadInit){this.hadInit=true;this.tempName="";this._bindFormValidation();this._bindEventHander();this._loadOptCatesTree();
...
}},_loadOptCatesTree:function(){var a=this;var b=CateOpt._CONS_.LIST_CATE_URL;FormUtils.loadData(b,function(c){if(c&&c.length){var g=new StringBuffer();
var f="";for(var d=0;d<c.length;d++){var e="";g.append("<li class='list-group-item' idVal='").append(c[d].id).append("' keyVal='"+c[d].key+"'><a href='javascript:void(0);'>").append(c[d].name).append("</a></li>");
...
}var f=d.find(".colorSpan").attr("colorVal");var c=ctx+"/gl/cate/cateOpt/saveAdd.htm";if(!b.trim()){DialogUtils.error(msgCode.ERROR,"请输入选项值");return;}var e={cateId:j,name:encodeURI(b.trim()),status:g,color:f?f:"#fff",sort:h};
FormUtils.submit(c,e,function(k){if(k.success){window.sessionStorage.setItem("system-CsBizTypes","");a._reloadCateItem(j,d);}else{if(k.msg=="repeat"){DialogUtils.error(msgCode.ERROR,"重复添加");
}else{DialogUtils.error(msgCode.ERROR,"添加失败");}}});});$(".folder-list").on("click","li",function(){$("#optContainer").empty();$(".folder-list li").removeClass("active");
$(this).addClass("active");var b=$(this).attr("idVal");a._loadOptions(b);});$("#addOptCateBtn").on("click",function(){$(".supportColorDiv").empty().append('<input type="checkbox" value="y" name="supportColor" placeholder="" class="form-control supportColorCheckbox" validateType="default">');
...
var e={id:c,cateId:j,name:encodeURI(b),status:g,color:f?f:"#fff",sort:h,createTime:i};FormUtils.submit(ctx+"/gl/cate/cateOpt/saveEdit.htm.htm",e,function(k){if(k.success){window.sessionStorage.setItem("system-CsBizTypes","");
DialogUtils.success("提示","更新成功");}else{DialogUtils.error("失败",k.msg);}},function(){DialogUtils.error("错误","后台异常，请稍后重试");});});},_reloadCateItem:function(c,b){var a=this;
